...
export default function HebrewTypeIndex() {
  ...
  return (
    <div dir="rtl" style={{ minHeight: "100vh", background: "var(--paper)", color: "var(--ink)", fontFamily: "'IBM Plex Sans Hebrew', sans-serif" }}>
      <style>{`
        @import url('https://fonts.googleapis.com/css2?family=Suez+One&family=IBM+Plex+Sans+Hebrew:wght@300;400;500;600;700&display=swap');
        :root {
          --paper: #F5F1E6;
          --paper-deep: #ECE6D4;
          --ink: #1C1A14;
        ...
        * { box-sizing: border-box; }
        .hti-wrap { max-width: 1240px; margin: 0 auto; padding: 36px 24px 80px; }
        .hti-head {
          border: 2px solid var(--line);
          background:
            radial-gradient(circle at 1px 1px, rgba(28,26,20,.10) 1px, transparent 0) 0 0/14px 14px,
            var(--paper-deep);
        ...
        .hti-head::after {
          ...
          border: 1px dashed rgba(28,26,20,.35); pointer-events: none;
        }
        .hti-kicker { font-size: 12px; letter-spacing: .12em; color: var(--ink-soft); margin: 0 0 6px; }
        .hti-title { font-family: 'Suez One', serif; font-size: clamp(30px, 5vw, 52px); line-height: 1.05; margin: 0; }
        .hti-title em { font-style: normal; color: var(--accent); }
        .hti-sub { margin: 10px 0 0; color: var(--ink-soft); font-size: 14.5px; max-width: 62ch; }
        .hti-bar { display: flex; gap: 10px; flex-wrap: wrap; align-items: center; margin: 18px 0 8px; position: relative; z-index: 1; }
        .hti-search {
          flex: 1 1 260px; border: 2px solid var(--line); background: var(--paper);
          padding: 11px 14px; font: inherit; font-size: 15px; outline: none;
        }
        .hti-search:focus { box-shadow: 4px 4px 0 var(--accent); }
        .hti-select {
          border: 2px solid var(--line); background: var(--paper);
          padding: 11px 12px; font: inherit; font-size: 14px; cursor: pointer;
        }
        .hti-clear {
          border: 2px solid var(--line); background: var(--ink); color: var(--paper);
          padding: 11px 16px; font: inherit; font-size: 14px; cursor: pointer; font-weight: 600;
        }
        .hti-clear:hover { background: var(--accent); border-color: var(--accent); }
        .hti-groups { display: flex; flex-direction: column; gap: 10px; margin-top: 8px; position: relative; z-index: 1; }
        .hti-group { display: flex; gap: 8px; flex-wrap: wrap; align-items: baseline; }
        .hti-glabel {
          font-size: 11px; font-weight: 700; letter-spacing: .1em;
          color: var(--ink-soft); min-width: 76px;
        }
        .chip {
          border: 1.5px solid var(--line); background: var(--paper);
          padding: 5px 12px; font: inherit; font-size: 13px; cursor: pointer;
          border-radius: 999px; transition: transform .06s ease;
        }
        .chip:hover { transform: translateY(-1px); }
        .chip.on { background: var(--ink); color: var(--paper); }
        .chip.on.fo { color: #fff; }
        .hti-meta { display: flex; justify-content: space-between; align-items: baseline; margin: 22px 2px 12px; gap: 12px; flex-wrap: wrap; }
        .hti-count { font-family: 'Suez One', serif; font-size: 20px; }
        .hti-count b { color: var(--accent); }
        .hti-grid {
        ...
        .card {
          border: 2px solid var(--line); background: var(--paper);
          padding: 16px 16px 14px; position: relative; display: flex; flex-direction: column; gap: 8px;
          transition: transform .08s ease, box-shadow .08s ease;
        }
        .card:hover { transform: translate(-2px, -2px); box-shadow: 5px 5px 0 var(--cardc, var(--accent)); }
        .card-top { display: flex; justify-content: space-between; align-items: center; gap: 8px; }
        .fo-tag {
          font-size: 11px; font-weight: 700; color: #fff; padding: 3px 9px; border-radius: 999px;
          background: var(--cardc); white-space: nowrap;
        }
        .yr { font-size: 12px; color: var(--ink-soft); font-variant-numeric: tabular-nums; }
        .card h3 { font-family: 'Suez One', serif; font-size: 26px; margin: 0; line-height: 1.1; }
        .lat { font-size: 12.5px; color: var(--ink-soft); letter-spacing: .02em; direction: ltr; text-align: right; margin-top: -4px; }
        .dsg { font-size: 13px; }
        .dsg span { color: var(--ink-soft); }
        .mini { display: flex; gap: 6px; flex-wrap: wrap; margin-top: 2px; }
        .mini i {
          font-style: normal; font-size: 11px; border: 1px solid rgba(28,26,20,.45);
          padding: 2px 8px; border-radius: 999px; color: var(--ink-soft);
        }
        .mini i.hot { border-color: var(--accent); color: var(--accent); font-weight: 600; }
        .mini i.vbc { border-style: dashed; border-color: var(--accent); color: var(--accent); }
        .chip.vb.on { background: var(--accent); border-color: var(--accent); }
        .mini i.free { background: var(--accent); border-color: var(--accent); color: #fff; font-weight: 700; }
        .card-foot { display: flex; justify-content: space-between; align-items: center; margin-top: auto; padding-top: 8px; border-top: 1px dashed rgba(28,26,20,.35); }
        .wlab { font-size: 12px; color: var(--ink-soft); }
        .goto { font-size: 13px; font-weight: 700; color: var(--ink); text-decoration: none; }
        .goto:hover { color: var(--accent); }
        .empty { border: 2px dashed var(--line); padding: 48px 24px; text-align: center; color: var(--ink-soft); grid-column: 1 / -1; }
        .hti-foot { margin-top: 28px; font-size: 12.5px; color: var(--ink-soft); border-top: 2px solid var(--line); padding-top: 12px; display:flex; justify-content: space-between; gap: 12px; flex-wrap: wrap; }
      `}</style>

      <div className="hti-wrap">
        <header className="hti-head">
          <p className="hti-kicker">אינדקס ◌ משאבים לסטודנטים לעיצוב גרפי</p>
          <h1 className="hti-title">אינדקס <em>הפונט העברי</em></h1>
          <p className="hti-sub">
          ...
          <div className="hti-bar">
            <input
              className="hti-search"
              placeholder="חיפוש פונט, שם לועזי או מעצב/ת…"
              value={query}
              onChange={(e) => setQuery(e.target.value)}
            />
            <select className="hti-select" value={designer} onChange={(e) => setDesigner(e.target.value)}>
              <option value="">כל המעצבים/ות</option>
              {designers.map((d) => <option key={d} value={d}>{d}</option>)}
            </select>
            <select className="hti-select" value={sort} onChange={(e) => setSort(e.target.value)}>
            ...
            {activeCount > 0 && (
              <button className="hti-clear" onClick={clearAll}>ניקוי ({activeCount})</button>
            )}
          </div>

          <div className="hti-groups">
            <div className="hti-group">
              <span className="hti-glabel">בית פונטים</span>
              {Object.entries(FOUNDRIES).map(([id, fo]) => (
                <button
                  key={id}
                  className={"chip fo" + (foundries.includes(id) ? " on" : "")}
            ...
            <div className="hti-group">
              <span className="hti-glabel">סגנון</span>
              {Object.entries(STYLES).map(([id, label]) => (
                <button key={id} className={"chip" + (styles.includes(id) ? " on" : "")} onClick={() => toggle(setStyles)(id)}>{label}</button>
              ))}
            </div>
            <div className="hti-group">
              <span className="hti-glabel">שימוש</span>
              {Object.entries(USES).map(([id, label]) => (
                <button key={id} className={"chip" + (uses.includes(id) ? " on" : "")} onClick={() => toggle(setUses)(id)}>{label}</button>
              ))}
            </div>
            <div className="hti-group">
              <span className="hti-glabel">מאפיינים</span>
              {Object.entries(TAGS).map(([id, label]) => (
                <button key={id} className={"chip" + (tags.includes(id) ? " on" : "")} onClick={() => toggle(setTags)(id)}>{label}</button>
              ))}
            </div>
            <div className="hti-group">
              <span className="hti-glabel">VIBE</span>
              {Object.entries(VIBES).map(([id, label]) => (
                <button key={id} className={"chip vb" + (vibes.includes(id) ? " on" : "")} onClick={() => toggle(setVibes)(id)}>{label}</button>
              ))}
            </div>
            <div className="hti-group">
              <span className="hti-glabel">תקופה</span>
              {ERAS.map((e) => (
                <button key={e.id} className={"chip" + (eras.includes(e.id) ? " on" : "")} onClick={() => toggle(setEras)(e.id)}>{e.label}</button>
              ))}
            </div>
          </div>
        </header>

        <div className="hti-meta">
          <div className="hti-count"><b>{results.length}</b> פונטים נמצאו</div>
          {(tags.length + vibes.length) > 1 && <div style={{ fontSize: 12, color: "var(--ink-soft)" }}>מאפיינים מסוננים יחד (וגם־וגם)</div>}
        </div>

        <main className="hti-grid">
          {results.length === 0 && (
            <div className="empty">
          ...
          {results.map((x, i) => {
            ...
            return (
              <article className="card" key={x.lat + i} style={{ "--cardc": fo.color }}>
                <div className="card-top">
                  <span className="fo-tag">{fo.name}</span>
                  <span className="yr">{x.y ?? "—"}</span>
                </div>
                <h3>{x.n}</h3>
                <div className="lat">{x.lat}</div>
                <div className="dsg"><span>עיצוב: </span>{x.d ?? "—"}</div>
                <div className="mini">
                  {x.st.map((s) => <i key={s}>{STYLES[s]}</i>)}
                  {x.tg.includes("free") && <i className="free">חינמי</i>}
                  {x.tg.includes("variable") && <i className="hot">וריאבלי</i>}
                  {x.tg.includes("trilingual") && <i className="hot">עב׳·ער׳·אנ׳</i>}
                  {x.tg.includes("bilingual") && !x.tg.includes("trilingual") && <i>דו־לשוני</i>}
                  {x.tg.includes("narrow") && <i>צר</i>}
                  {x.tg.includes("wide") && <i>רחב</i>}
                  {x.tg.includes("rounded") && <i>מעוגל</i>}
                  {x.tg.includes("nostalgic") && <i>נוסטלגי</i>}
                  {x.tg.includes("classic") && <i>קלאסי</i>}
                  {x.vb.map((v) => <i key={v} className="vbc">{VIBES[v] || v}</i>)}
                </div>
                <div className="card-foot">
                  <span className="wlab">{x.w ? `${x.w} משקלים` : "מס׳ משקלים —"}</span>
                  <a className="goto" href={x.url} target="_blank" rel="noreferrer">לעמוד הפונט ↖</a>
                </div>
              </article>
            );
          })}
        ...
        <footer className="hti-foot">
          <span>הנתונים נאספו מאתרי בתי הפונטים. שדות המסומנים ב־“—” טרם אומתו — עריכה במערך FONTS שבקוד.</span>
          <span>אינדקס · index-design-hit</span>
        </footer>
      </div>
    </div>
  );
